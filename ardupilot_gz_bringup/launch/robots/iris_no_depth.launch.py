...
def launch_state_pub_with_bridge(context):
    ...
    sdf_file = os.path.join(
        pkg_ardupilot_gz_description, "models", "iris_no_depth", "model.sdf"
    )

    with open(sdf_file, "r") as infp:
        robot_desc = infp.read()

    ros_gz_bridge_config = "iris_no_depth.yaml"

    log = LogInfo(msg="Using iris_no_depth model and iris_no_depth.yaml")
...
def generate_launch_arguments():
    ...
    return [
        ...
        DeclareLaunchArgument("z", default_value="0.3", description="Initial 'z' position (m)."), # Adjusted for Iris
...
def generate_launch_description():
    ...
    # TODO: Create or verify existence of 'gazebo-iris.parm'
    # For now, using gazebo-iris.parm as a placeholder if iris specific one is not available
    iris_params_file = os.path.join(pkg_ardupilot_sitl, "config", "default_params", "iris-depth.param")
    if not os.path.exists(iris_params_file):
        iris_params_file = os.path.join(pkg_ardupilot_sitl, "config", "default_params", "gazebo-iris.parm")
        print(f"Warning: gazebo-iris.parm not found, using {iris_params_file} as fallback.")
    ...
    #     }.items(),
    # )
    sitl_apm = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution([
                FindPackageShare("ardupilot_sitl"),
                "launch",
        ...
        launch_arguments={
            "command": "arducopter",
            "model": "gazebo-iris", # SITL model type, Gazebo uses the SDF.
            "defaults": iris_params_file,
            "model": "json",
            "synthetic_clock": "True",
            "sim_address": "127.0.0.1",
    ...
    #     shell=True,
    # )
    # Run command to run mavproxy
    mavros_node = Node(
        package="mavros",
        executable="mavros_node",
        output="screen",
        parameters=[
            {"fcu_url": "udp://127.0.0.1:14550@"},#FIX IS NOT WORKING
            {"gcs_url": "udp://@localhost:14551"}
            # Add other MAVROS params as needed, e.g., plugin_allowlist/blocklist
        ]
    )
