...
# Make all NVIDIA GPUs visible to the container by default.
# If $NVIDIA_VISIBLE_DEVICES is already set externally, use that value instead.
# This is important for hardware-accelerated Gazebo simulation or GPU-enabled ROS environments.
ENV NVIDIA_VISIBLE_DEVICES \
    ${NVIDIA_VISIBLE_DEVICES:-all}

# Extend NVIDIA driver capabilities to include 'graphics'.
# If capabilities are already defined, they are appended; otherwise it enables only graphics.
# Required when using GPU-rendered simulation tools like Gazebo Harmonic or RViz.
